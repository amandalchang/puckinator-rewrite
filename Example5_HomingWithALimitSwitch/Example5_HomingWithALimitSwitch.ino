...
void loop() 
{
  ...
  //
  stepper.setStepsPerRevolution(1600);    // 8x microstepping
  ...
  //
  const float homingSpeedInRevolutionPerSec = 0.1;
  const float maxHomingDistanceInInches = 10;   // since my lead-screw is 38cm long, should never move more than that
  const int directionTowardHome = 1;        // direction to move toward limit switch: 1 goes positive direction, -1 backward
  
  if(stepper.moveToHomeInRevolutions(directionTowardHome, homingSpeedInRevolutionPerSec, maxHomingDistanceInInches, LIMIT_SWITCH_PIN) != true)
  {
    //
